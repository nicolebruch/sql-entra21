...
SELECT e.nm_municipio, MAX(c.dt_nascimento) AS idade_mais_jovem
FROM cliente c JOIN endereco e 
ON c.cd_endereco = e.id
GROUP BY e.nm_municipio;
...
SELECT c.nm_categoria, SUM(p.vl_produto) AS soma_total
FROM categoria c JOIN produto p 
ON c.id = p.cd_categoria
GROUP BY c.nm_categoria;
